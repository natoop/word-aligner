...
- `repair.min_score_gain`：`span-aware` 扩展及复合修复岛扩展所需的最小分数增益，默认 `0.05`。
...
精炼已有锚点的分句前，服务会删除跨越已对齐标点锚点的低置信度词汇链接；锁定的规则关系以及达到边界锁定置信度的链接不会被删除。如果删除后相邻的同一组单调链接之间在两侧都出现未对齐内容，服务只精炼这个有边界的局部缺口。两个相邻缺口可以在跨越弱内部链接且池化 Embedding 分数通过规范化 `min_score_gain` 测试时合并。周围稳定链接仍保持原子关系，确实没有译文的虚词也可以继续保持未对齐。

两侧覆盖率达到 100% 也不代表 token 角色一定正确。在已有锚点限定的分句内，如果一段连续一对一链接的目标端顺序严格反向，服务会把它收拢成一个局部精炼短语组。至少包含三条证据链接的稀疏严格倒序序列，在不超过两个配置局部 Span 时可以使用其连续包围区间。因此既能把 `原子/链接 → links/atomic` 改为 `原子链接 ↔ atomic links`，也能把 `Programme for the Endorsement of Forest ↔ 森林认证认可计划` 表示为完整短语，同时保留周围稳定链接。
...
确定性的数量规范化还会处理 tokenizer 边界不同的百分比。源端拆分 Span `[70, %]` 和目标端合并 token `[70%]` 会形成一组完整的 `many-to-one`、`origin: "rule"` 关系，相似度和置信度均为 `1.0`。同一句中的重复数值按出现顺序配对，因此 `70%`、`30%` 和 `100%` 不会互相串联。
