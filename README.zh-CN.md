...
- `repair.min_score_gain`：`span-aware` 扩展所需的最小池化 Embedding 分数增益，默认 `0.05`。
...
原子链接的 `origin` 用于区分来源：`model` 是所选 SimAlign 方法的原始关系，`rule` 是占位符或标签的精确规则关系，`repaired` 是保守修正补充的关系。
